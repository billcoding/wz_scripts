    ...
    if (status == 0) {
        ...
        if (cm.getParty() == null) { // No Party
            ...
            for (var i = 0; i < party.size(); i++) {
                if (party.get(i).getLevel() >= 最小等级 && party.get(i).getLevel() <= 最大等级)
                    levelValid++;
            }
            if (inMap < 最小人数 || inMap > 最大人数) {
                cm.sendOk("你的队伍人数不足"+最小人数+"人.请把你的队伍人员召集到废气都市在进入副本.");
                //cm.sendOk("Your party is not a party of "+最小人数+". Please make sure all your members are present and qualified to participate in this quest. I see #b" + inMap + "#k of your party members are in Kerning. If this seems wrong, #blog out and log back in,#k or reform the party.");
                cm.dispose();
            } else if (levelValid != inMap) {
                cm.sendOk("请确保你的队伍人员最小等级在 "+最小等级+" 和 "+最大等级+"之间，且队友都在同一张地图！");
                cm.dispose();
            } else {
