	...
	if (mode == -1) {
		...
		if (status == 0) {
			...
			if(selection == 0){		
				...
				cm.sendGetNumber("注:每次最少二十万,赌友你要下注多少？\r\n", 200000, 200000,20000000);
		...
		//------------第二次要牌------------------//
			...
			if(selection == 2){
				...
				if((xianScore<22 && xianScore<=zhuangScore && zhuangScore<22) || (xianScore>=22 && zhuangScore>=22)){
					...
					//cm.喇叭("[娱乐21点] : 土豪玩家 "+cm.getPlayer().getName()+" 在娱乐中心21点博弈中大获全胜恭喜恭喜！")
					cm.gainMeso(selectionMeso);
					cm.dispose();	
				}
			}
		...
		//-----------第三次要牌---------------
			...
			if(selection == 4){
				...
				if((xianScore<22 && xianScore<=zhuangScore && zhuangScore<22) || (xianScore>=22 && zhuangScore>=22)){
					...
					cm.sendOk("#fUI/UIWindow/Quest/reward#\r\n  庄家的点数为 #r#e"+zhuangScore+"#n#k,您的点数为 #r#e"+xianScore+"#n#k,您赢了。");
					//cm.全服黄色喇叭("[娱乐21点] : 土豪玩家 "+cm.getPlayer().getName()+" 在娱乐中心21点博弈中大获全胜恭喜恭喜！")
					cm.gainMeso(selectionMeso);
					cm.dispose();	
				}
			}
